...
codeBlocks.forEach((code) => {
  ...
  // Cleanup trailing empty lines generated by markdown/shiki
  const codeElement = code.querySelector('code');
  if (codeElement) {
      const lineSpans = codeElement.querySelectorAll('.line');
      if (lineSpans.length > 0) {
          const lastLine = lineSpans[lineSpans.length - 1];
          // Remove if it's completely empty
          if (lastLine.textContent.trim() === '') {
              lastLine.remove();
          }
      }
      // Remove trailing text nodes containing only whitespace which can add height
      if (codeElement.lastChild && codeElement.lastChild.nodeType === Node.TEXT_NODE && codeElement.lastChild.textContent.trim() === '') {
          codeElement.lastChild.remove();
      }
  }

  const textContent = codeElement?.textContent || '';
  let linesCount = codeElement?.querySelectorAll('.line').length;
  if (!linesCount) {
      linesCount = Math.max(1, textContent.trimEnd().split('\n').length);
  }
  const linesText = `${linesCount} ${linesCount > 1 ? 'Lines' : 'Line'}`;
